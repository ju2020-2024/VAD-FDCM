...
def test(test_loader, model, gt, args, extract_features=False, gt_path=False):
    model.eval()
    ...
    with torch.no_grad():
        for inputs in test_loader:
            ...
            # x_input, x_out, h_common, h_specific, gate = out["csfd"]
            #
            ...
            s_1_sigmoid = torch.sigmoid(s_1)
            s3_sigmoid = torch.sigmoid(s3)

            s_1_mean = torch.mean(s_1_sigmoid, dim=0)
            s3_mean = torch.mean(s3_sigmoid, dim=0)

            all_s1.append(s_1_mean.cpu())
            all_s3.append(s3_mean.cpu())
        ...
        all_s3 = torch.cat(all_s3, dim=0).numpy()
        all_s1 = np.repeat(all_s1, 16)
        all_s3 = np.repeat(all_s3, 16)
        plot_fig_zhe_xian(
            video_ranges,
            file_paths,
            "/home/stu2023/jj/fig_zhe_xian",
            all_s1,
            all_s3,
            gt
        )
        ...
        if gt_path:
            ...
            generate_xd_json_from_gt(gt, video_ranges, file_paths,
                                     '/home/stu2023/jj/project/VAD_new_vision/list/xd_test_gt.json')
            anomap(predict_dict, 'xd', '/home/stu2023/jj/project/vad2/xd/result/Fig/xd_GT-png')
